...
//                                            constrainedToSize:maxSize lineBreakMode:detailsLabel.lineBreakMode];
    CGRect detailsRect = [detailsLabel.text boundingRectWithSize:maxSize
                                                         options:NSStringDrawingUsesLineFragmentOrigin
                                                      attributes:@{NSFontAttributeName:detailsLabel.font}
                                                         context:nil];
    
	totalSize.width = MAX(totalSize.width, detailsRect.size.width);
	totalSize.height += detailsRect.size.height;
	if (detailsRect.size.height > 0.f && (indicatorF.size.height > 0.f || labelSize.height > 0.f)) {
		totalSize.height += kPadding;
	}
	...
	detailsLabelF.origin.x = roundf((bounds.size.width - detailsRect.size.width) / 2) + xPos;
	detailsLabelF.size = detailsRect.size;
